...
- the item-29 managed `packet build -> one inference` boundary passed per-cycle safety, but two natural cadences grew pending backlog by three each; its timer is disabled with append-only state preserved, and the protected-copy-proven backlog-deferral runner is installed inactive before guarded resume
