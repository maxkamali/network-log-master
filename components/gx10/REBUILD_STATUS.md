...
Item 29 is in progress. The published backlog correction passed protected-copy rehearsal, exact inactive installation, protected resume, and one natural drain before one terminal invalid output disabled reasoning. Copy diagnosis isolated action text equal to a risk label. Immutable-ID reuse was correctly refused; two separately versioned `r2` copies proved the runtime ignores `not/enum`. Exact published `incident-assessment-v2-r3` passed all 140 tests on GX10, and its portable 25-character action minimum made the exact failed packet produce one valid protected-copy result through the unchanged strict validator. The exact-old-hash installer then replaced only the runtime config, schema, caller, and managed runner while reasoning remained disabled. The full reasoning digest stayed unchanged. Production now has 12 revised-version pending packets, one original prompt registration, six original runs, five successes/results, one historical safe failure, zero `STARTED` rows/restarts, and deterministic zero lag. Fresh-backup protected `r3` drain is next.
