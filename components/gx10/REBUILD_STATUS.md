...
Item 29 is in progress. The published backlog correction passed protected-copy rehearsal, exact inactive installation, protected resume, and one natural drain. The second corrected natural cadence again kept packets fixed and consumed one reservation, but strict output validation rejected the model response. One terminal `INVALID_OUTPUT` run and no result were stored; invalid content was not stored. Reasoning was disabled with 12 packets, six old-version pending, six runs, five successes/results, one safe failure, zero `STARTED` rows/restarts, and deterministic zero lag. A protected-copy retry of the exact failed packet reproduced the rejection; a second isolated diagnostic safely classified one generated action whose text exactly equaled a risk label. The first strengthened-schema copy gate then correctly refused reuse of the immutable old prompt-version registration before any reservation/model call. The corrected 140-test candidate registers `incident-assessment-v2-r2`, prohibits risk labels as action text while preserving the strict caller check, and provides exact-old-hash upgrades only for the runtime config, schema, caller, and managed runner. It preserves all original-version state and exposes 12 revised-version pending packets. It is not installed; exact remote staging and same-packet protected-copy success are the next gates.
