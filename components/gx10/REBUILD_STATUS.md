...
The protected activator now passes 158 tests and an exact staged retained-copy rehearsal: reasoning was synthetically paused/restored, all five reasoning tables retained one exact digest, one initial local cycle created 12 files for 12 results, independent inactive/active cardinality gates passed, and only simulated systemd changed. Exact working-system activation remains next; credentials and collector transport remain absent.
