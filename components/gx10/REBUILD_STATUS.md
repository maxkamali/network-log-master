...
Item 27 has a repository-only deterministic wake-policy and compact-packet candidate. Its append-only one-table schema and builder implement fixed reason priorities/cooldowns, lifecycle/special-condition/meaningful-update decisions, deterministic versioned packet identity, bounded fact-only packets, existing-packet tamper detection, and replay-safe no-op behavior. Eight focused tests pass within the 88-test GX10 suite. No item-27 artifact, schema, packet, invocation, Ollama call, or result producer exists on the working system.
