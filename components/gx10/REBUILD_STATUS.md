...
Item 28 is complete. The smallest captured model passed structural output but failed urgency/action calibration and its invalid output was safely not stored. Validation was strengthened with deterministic severity alignment, 50–95 action-required confidence, packet-derived tag allowlisting, meaningful action text, and approval labels for change verbs. The exact `gemma4:latest` manifest/config passed three isolated synthetic cases. On the protected current-state copy, guarded migration/empty rollback/reapply passed, four sanitized packets were built, and success/invalid/unavailable/interrupted/no-op states each behaved exactly; one canonical result was stored and deterministic copy truth was unchanged. Twenty-one focused tests pass within the 115-test GX10 suite. The exact inference schema and caller/configuration/prompt/output-schema artifacts are now installed on the working system under a protected pre-inference backup but remain empty and unscheduled. A later cadence reverified exact hashes/schema/backup, zero packet/model/prompt/run/result rows, zero caller scheduler references, both deterministic cursors at zero lag, both existing timers active, zero correlation restarts, and no production inference.

Item 29 is in progress. The managed `packet build -> one local inference` runner/service/timer validates exact item-27/item-28 hashes, holds a separate single-cycle lock, refuses unreconciled `STARTED` reservations, permits at most one run and one result per cadence, and exposes aggregate packet backlog plus run/result health without identities or content. Twenty-one focused tests pass within the 136-test GX10 suite. The exact published candidate passed protected current-state-copy real-model success, invalid-response safe failure, interruption, retry refusal, filled-backlog no-op, bounded advancement, independent verifier, and deterministic-truth isolation gates. Its initial exact runner/service/timer/private binding was installed inactive after real `systemd-analyze` verification. Pre-activation review caught that the boot-relative timer could fire immediately when enabled on the long-running host; activation was not attempted. The published correction uses a start-relative five-minute initial delay and was atomically applied only over the exact-byte inactive predecessor. Post-upgrade verification passed with zero packet/model/prompt/run/result rows, zero restarts, and no inference. Protected initial-cycle activation is the next gate.
