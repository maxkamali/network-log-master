...
Item 29 is in progress. The published backlog correction passed protected-copy rehearsal, exact inactive installation, protected resume, and one natural drain. The second corrected natural cadence again kept packets fixed and consumed one reservation, but strict output validation rejected the model response. One terminal `INVALID_OUTPUT` run and no result were stored; invalid content was not stored. Reasoning was disabled with 12 packets, six old-version pending, six runs, five successes/results, one safe failure, zero `STARTED` rows/restarts, and deterministic zero lag. Protected-copy diagnosis isolated one action whose text exactly equaled a risk label. Immutable-ID reuse was correctly refused; two separately versioned `r2` copies proved the runtime ignores `not/enum`. Exact published `incident-assessment-v2-r3` passed all 140 tests on GX10, and its portable 25-character action minimum made the exact failed packet produce one valid protected-copy result through the unchanged strict validator. Deterministic truth and production were unchanged. The candidate provides exact-old-hash upgrades only for the runtime config, schema, caller, and managed runner and exposes 12 revised-version pending packets. It is not installed; checkpoint publication and exact inactive upgrade are next.
