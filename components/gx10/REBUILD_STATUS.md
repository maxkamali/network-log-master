...
Item 29 is in progress. Two natural cadences exposed packet admission faster than inference, so only reasoning was disabled with append-only state preserved. The published 138-test correction defers the builder while pending exists. It passed protected-copy rehearsal, exact inactive installation, and protected production resume. The first independent timer-driven drain also passed: packets remained 12, pending fell 8→7, successful runs/results rose 4→5, and failure/`STARTED`/restart/lag counts remained zero. Additional natural drain cadences are next.
