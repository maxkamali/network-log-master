...
Item 29 is in progress. The managed `packet build -> one local inference` runner/service/timer validates exact item-27/item-28 hashes, holds a separate single-cycle lock, refuses unreconciled `STARTED` reservations, permits at most one run and one result per cadence, and exposes aggregate packet backlog plus run/result health without identities or content. Its installer leaves the timer disabled, its activator creates a protected SQLite backup before one bounded initial cycle, and any activation failure disables only reasoning. Twenty focused tests pass within the 135-test GX10 suite. The exact published candidate passed protected current-state-copy real-model success, invalid-response safe failure, interruption, retry refusal, filled-backlog no-op, bounded advancement, independent verifier, and deterministic-truth isolation gates. No item-29 artifact, unit, configuration, packet, run, result, or schedule is installed on the working system; publish the copy evidence before inactive installation.
