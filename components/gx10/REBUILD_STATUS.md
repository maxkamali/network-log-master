...
The inactive managed-package candidate adds an exact-hash runner, private-network/Unix-only oneshot and disabled timer, guarded installer, independent verifier, and six focused management tests for 157 total. The exact staged tree passed on GX10. An isolated copy rehearsal installed exact bytes/directories/configuration while simulating systemd only, held the timer disabled, ran the installed producer over the 12-result protected copy, preserved delivered suppression and the source hash, and proved complete cleanup after forced post-install verification failure. Working-system installation remains the next gate; no credential or transport exists.

The first working-system install safely stopped before preflight on a public-default/private-runtime path mismatch and left zero targets/units. A corrected exact package derives the installed managed-reasoning database/root and repeated all 157 remote plus isolated success/failure gates. Its working-system inactive install then passed with 14 current results, zero ready/delivered files, disabled timer, never-invoked service, effective private network/Unix-only/write-scope restrictions, healthy zero-lag reasoning/correlation/pipeline schedules, and no credential or transmission. Protected local-only activation is next.

The protected activator now passes 159 tests and two exact staged retained-copy rehearsals. First working-system activation created 15 ready files but safely stopped when the root-run verifier required service-owned files to match root UID. Outbox remained disabled, reasoning was restored, files were retained, and nothing transmitted. The corrected verifier checks the derived service UID/GID and the activator safely resumes exact populated-disabled state; a focused ownership test plus second full copy rehearsal pass. Exact working-system resume is next.
...
- 157 synthetic tests passing at the current item-30 inactive-package candidate
