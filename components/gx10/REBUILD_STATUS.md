...
Item 29 is in progress. The managed `packet build -> one local inference` runner/service/timer validates exact item-27/item-28 hashes, holds a separate single-cycle lock, refuses unreconciled `STARTED` reservations, permits at most one run and one result per cadence, and exposes aggregate packet backlog plus run/result health without identities or content. Twenty-one focused tests pass within the 136-test GX10 suite. Protected activation created a validated backup and completed one successful run/result. Two later natural cadences each safely added one successful run/result with zero failures, `STARTED` rows, restarts, or deterministic lag, but each also built four new packets. Pending backlog grew from three to six to nine, failing stability. Only the managed reasoning timer was disabled; 12 packets and three successful results remain append-only, and fetch/ingest plus deterministic correlation are healthy. A bounded admission/coalescing correction and new cadence gates are next.
