...
Item 29 is in progress. The published 138-test backlog correction passed protected-copy rehearsal, exact inactive installation, protected resume, and one natural drain. The second corrected natural cadence again kept packets fixed and consumed one reservation, but strict output validation rejected the model response. One terminal `INVALID_OUTPUT` run and no result were stored; invalid content was not stored. Reasoning was disabled with 12 packets, six pending, five successes/results, one safe failure, zero `STARTED` rows/restarts, and deterministic zero lag. Protected-copy reproduction of the same failed reservation is next.
