...
Item 29 is in progress. A repository-only managed `packet build -> one local inference` runner/service/timer candidate validates exact item-27/item-28 hashes, holds a separate single-cycle lock, refuses unreconciled `STARTED` reservations, permits at most one run and one result per cadence, and exposes aggregate packet backlog plus run/result health without identities or content. Its installer leaves the timer disabled, its activator creates a protected SQLite backup before one bounded initial cycle, and any activation failure disables only reasoning. An explicit injection-only transport seam permits private-copy safe-failure/interruption rehearsal without changing the production CLI path. Twenty focused tests pass within the 135-test GX10 suite. No item-29 artifact, unit, configuration, packet, run, result, or schedule is installed on the working system; protected-copy rehearsal remains the next gate.
