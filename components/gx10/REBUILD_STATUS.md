...
Item 29 is in progress. Protected activation and per-cycle inference safety passed, but two natural cadences each built four packets while consuming one, growing pending backlog from three to six to nine. Only reasoning was disabled; 12 packets and three successful results remain append-only with zero failures/`STARTED` rows/restarts. A 138-test correction now defers the builder whenever exact-version pending work exists, requires packet count to remain fixed and pending to fall by exactly one, and resumes construction only at pending zero so intervening incident changes coalesce. The installer accepts only the exact old runner for atomic replacement/rollback. Twenty-three focused tests pass. Protected current-state-copy rehearsal remains required before inactive production upgrade and new cadence gates.
