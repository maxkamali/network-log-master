...
Item 29 is in progress. Protected activation and per-cycle inference safety passed, but two natural cadences each built four packets while consuming one, growing pending backlog from three to six to nine. Only reasoning was disabled; 12 packets and three successful results remain append-only with zero failures/`STARTED` rows/restarts. The published 138-test correction defers the builder whenever exact-version pending work exists, requires packet count to remain fixed and pending to fall by exactly one, and resumes construction only at pending zero. A protected caught-up production-state copy passed one real-model drain: packets 12→12, pending 9→8, runs/results 3→4, zero failure/`STARTED`, and deterministic truth unchanged. Exact inactive runner upgrade is next.
