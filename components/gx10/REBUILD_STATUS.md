...
Item 28 has published initial repository/guard candidates and a calibrated correction ready for publication. The smallest captured model passed structural output but failed urgency/action calibration and its invalid output was safely not stored. Validation was strengthened with deterministic severity alignment, 50–95 action-required confidence, packet-derived tag allowlisting, meaningful action text, and approval labels for change verbs. The exact `gemma4:latest` manifest/config then passed all three isolated synthetic OSPF/interface/BGP loopback cases, canonical result hashes, and immediate no-op. The guard still requires exact pre/post schema and artifact hashes, the exact unscheduled item-27 builder, protected pre-inference backup, zero caller scheduler references, atomic failure cleanup, and empty-state-only rollback. Twenty-one focused tests pass within the 115-test GX10 suite. No item-28 schema/artifact/run/result is installed on the working system and no production packet has been created or invoked.
