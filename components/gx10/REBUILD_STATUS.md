...
Item 29 is in progress. Two natural cadences exposed packet admission faster than inference, so only reasoning was disabled with 12 packets, nine pending, and three successful results preserved. The published 138-test correction defers the builder while pending work exists. A protected current-state copy passed one real-model drain with packets 12→12, pending 9→8, runs/results 3→4, and deterministic truth unchanged. The corrected runner was then atomically installed only over the exact predecessor while disabled; all reasoning rows retained the same digest and counts. Protected resume and new natural drain cadences are next.
