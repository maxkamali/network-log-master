...
Item 26 has a published inactive candidate and passing private working-database-copy rehearsal for a separately managed offline `projection -> incident` service/timer. It preserves fetch/ingest unchanged, enforces exact stage hashes and single-cycle locking, converges both cursors in bounded passes, exposes structured health counts, and has guarded inactive install, backfill-before-enable activation, verification, and state-preserving disable controls. Inactive working-system install, live backfill/activation, and multi-cadence gates remain pending.
