...
- first live result transport and durable acceptance: `PARTIAL PASS` with exact one-file ready-to-delivered transition, unchanged private cross-host/ledger/ready name and digest, corrected gate installed under exact predecessor backup, one immutable acceptance row, enabled/active gate timer, disabled sender timer, 60 results/59 ready/one delivered, zero deterministic lag, and healthy services; direct ClickHouse row/`raw_json` proof awaits explicitly approved local read-only credential use
