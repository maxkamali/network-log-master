...
The end-to-end working system is complete through execution-order item 40. Post-closure item 41 is the single `NEXT` item. Its validated repository candidate is a presentation-only rolling interface-flap filter; production dry-run, protected two-organization replacement, exact reread, live queries/drilldown, and health gates remain. The forward-only 24-hour OSPF/BGP monitoring policy, backward-compatible lifecycle producer version 2 recurrence projection, additive collector column, enhanced-dashboard `MONITORING`/issue-occurrence presentation, isolated NOC Viewer access boundary, one-minute NOC rotation playlist, and compact one-click Explore drilldowns are active under protected predecessors.
...
- Item 39 adds `View matching logs` to every cell in all three enhanced event tables. The selected incident ID resolves authoritative device/entity/protocol/event-family and incident timestamps, then opens at most 1,000 newest-first read-only `grafana_logs` rows with 15 minutes of surrounding context. Main and NOC copies use organization-local Explore links; both were protected, dry-run, replaced, reread exact, and query-validated without changing the playlist or any incident/data-path state.
