...
The final repository rebuild milestone is published. Item 29 passed protected-copy, inactive-install/correction, and protected activation gates, but two natural cadences each admitted four packets while consuming one. Reasoning is disabled with 12 packets, nine pending, and three successful results preserved. The published 138-test correction defers construction whenever pending exists and requires an exact one-packet drain. A protected caught-up production-state copy passed one real-model cycle with packets 12→12, pending 9→8, runs/results 3→4, zero failure/`STARTED`, and unchanged deterministic truth. Publish that rehearsal, then atomically upgrade only the exact old runner while still disabled before protected resume and new cadence gates. Do not add collector result production in item 29.
