...
The final repository rebuild milestone is published. Item 29's backlog correction passed protected-copy rehearsal, exact inactive installation, protected resume, and one natural drain. The second natural drain kept packets fixed and pending fell 7→6, but strict validation stored one terminal `INVALID_OUTPUT` run and no result/content. Reasoning is disabled with 12 packets, six old-version pending, six runs, five successes/results, one safe failure, zero `STARTED` rows/restarts, and zero deterministic lag. Protected-copy replay reproduced the failure and safely classified it as action text exactly equal to a risk label. The first strengthened-schema copy gate correctly refused to reuse the immutable old prompt-version ID before model invocation. The corrected 140-test candidate creates prompt revision `incident-assessment-v2-r2`, excludes risk labels at generation time without weakening validation, and starts a clean 12-packet revised-version backlog. Publish/stage the exact candidate and require same-packet protected-copy success before any inactive four-artifact production upgrade or protected resume. Do not add collector result production in item 29.
