...
The final repository rebuild milestone is published. Item 29's backlog correction passed protected-copy rehearsal, exact inactive installation, protected resume, and one natural drain. The second natural drain kept packets fixed and pending fell 7→6, but strict validation stored one terminal `INVALID_OUTPUT` run and no result/content. Reasoning is disabled with 12 packets, six old-version pending, six runs, five successes/results, one safe failure, zero `STARTED` rows/restarts, and zero deterministic lag. Protected-copy replay reproduced and classified action text equal to a risk label. Immutable-ID reuse was correctly refused; separately versioned `r2` proved the runtime ignores `not/enum`. Exact published `incident-assessment-v2-r3` passed 140 GX10 tests and the same failed packet produced one valid protected-copy result through the portable 25-character generation minimum and unchanged strict validator. Publish this copy checkpoint, then install only the exact four compatibility bytes inactive and verify unchanged production state before a protected `r3` drain. Do not add collector result production in item 29.
