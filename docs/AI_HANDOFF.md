...
The final repository rebuild milestone is published. Item 29's backlog correction passed protected-copy rehearsal, exact inactive installation, protected resume, and one natural drain. The second natural drain kept packets fixed and pending fell 7→6, but strict validation stored one terminal `INVALID_OUTPUT` run and no result/content. Reasoning is disabled with 12 packets, six pending, six runs, five successes/results, one safe failure, zero `STARTED` rows/restarts, and zero deterministic lag. Protected-copy replay reproduced the failure and safely classified it as action text exactly equal to a risk label. The 140-test compatibility candidate excludes those labels at generation time without weakening validation. Publish/stage the exact candidate and require same-packet protected-copy success before any inactive production upgrade or protected resume. Do not add collector result production in item 29.
