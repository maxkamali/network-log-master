...
26. `NEXT` — design and validate a managed canonical-projection/incident invocation boundary with exact ordering, backlog behavior, telemetry, resource limits, failure isolation, disable/rollback controls, initial backfill, and steady-state proof. Do not add Ollama invocation or result production to this gate.
