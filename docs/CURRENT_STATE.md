...
30. `NEXT` — complete the separately gated GX10 result-return boundary. Configured-inactive transport and the first-live checkpoint are published. One exact delivered-file replay passed: the collector quarantined it specifically as already accepted, retained one immutable ledger/ready identity, and ClickHouse retained exactly one byte-equivalent row. Full postchecks retain zero deterministic lag, healthy schedules, and a disabled sender timer. Next publish exact-replay closure, then complete the separately bounded same-name divergent isolation before considering a recurring sender schedule.
