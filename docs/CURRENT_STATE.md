...
26. `NEXT` — complete private-copy and live activation gates for the implemented managed canonical-projection/incident boundary. Exact ordering, bounded cursor convergence, structured telemetry, resource limits, fetch/ingest failure isolation, inactive installation, initial backfill-before-timer activation, and state-preserving disable controls are implemented locally; Ollama invocation and result production remain excluded.
