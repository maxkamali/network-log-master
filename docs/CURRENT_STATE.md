...
Post-closure NOC presentation correction: `DONE` under item 35. The live enhanced dashboard now uses `entity_type = interface` as the exclusive Interface Flaps presentation rule, adds the latest stored AI summary with deterministic fallback as Active Event detail, and labels the resolved statistic simply `Resolved`. Protected replacement and exact reread passed for the enhanced resource only; the current active split is two non-interface events and 34 interface events, with zero interface entities in Active Events.

Post-closure OSPF/BGP monitoring window: `IN PROGRESS` under item 36. The repository candidate retains confirmed OSPF/BGP incidents in deterministic `RECOVERING` state for 24 continuous healthy hours, presents that state as `MONITORING`, reopens the same correlated incident on a relapse, and exports a dedicated recurrence counter through backward-compatible lifecycle producer version 2. Existing resolved history remains unchanged. Production still uses the item-35 policy pending public checkpoint and protected deployment.
...
- collector-side write-only AI-result return boundary and validation/ingestion path, with an active durable accepted-filename/content ledger
- deterministic GX10 incident-state outbox using the same bounded write-only transport and a collector-exclusive lifecycle ingestion route
...
- six Grafana 13 dashboard resources, including the unchanged original AI dashboard and deterministic enhanced NOC workflow
...
The clean-machine runtime installer restores the six captured dashboards after HTTPS health and datasource verification, runs the independent dashboard verifier, and executes all thirteen current original/enhanced queries through Grafana's configured read-only ClickHouse datasource. Automatic replacement is intentionally not enabled; an unexpected existing divergent dashboard causes the installer to fail rather than overwrite it.
...
28. `DONE` — calibrated the strict versioned Gemma reasoning boundary, passed synthetic and protected production-state-copy success/invalid/unavailable/interrupted/idempotency gates, published the copy checkpoint, and installed only the exact inference schema/caller/configuration/prompt artifacts under a protected backup. At item-28 closure, production retained zero packet/model/prompt/run/result rows, zero caller scheduler references, and zero inference invocations; item 29 later began separately managed invocation.
...
36. `NEXT` — publish and independently verify the protocol-monitoring candidate; add the collector recurrence column and version-2 gate before any new producer record; atomically upgrade the GX10 engine/outbox/sender chain under protected predecessors; prove confirmed OSPF/BGP recovery stays active as `MONITORING` for 24 continuous healthy hours, relapse reopens the same incident and increments issue occurrences, existing resolved history is unchanged, legacy version-1 lifecycle files remain valid, all thirteen Grafana queries pass, and unrelated schedules/data paths remain healthy.

The end-to-end target remains complete through item 35. Item 36 is the single `NEXT`; it changes only the deterministic recovery-monitoring policy and lifecycle presentation/recurrence projection for forward operation, without rewriting existing resolved history or changing model authority.
