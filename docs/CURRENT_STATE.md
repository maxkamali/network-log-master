...
29. `NEXT` — publish and independently verify the successful `r3` same-packet protected-copy checkpoint, then atomically install only the four exact runtime-config/schema/caller/runner compatibility bytes while reasoning remains disabled. Require unchanged production row digests and revised-version pending 12 before a fresh protected backup and exactly one bounded `r3` drain. All old runs/results and the historical terminal failure remain immutable evidence. Collector result return remains out of scope.
