...
31. `NEXT` — perform the final production closure audit. Let the currently settling natural deliveries pass through the unchanged gate, verify delivered/accepted/in-flight cardinality, prove every durably accepted ready result has exactly one complete ClickHouse row, recheck both hosts and GitHub/documentation consistency, then publish the end-to-end target closure without deleting quarantine or rollback evidence.
