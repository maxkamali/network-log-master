...
Post-closure NOC presentation correction: `IN PROGRESS` under item 35. Aggregate production evidence showed 22 active `ethport` interface incidents with zero recorded state changes were classified outside the flap queue, while 12 interface incidents with changes were classified inside it. The repository candidate makes `entity_type = interface` the exclusive Interface Flaps presentation rule, adds the latest AI summary with deterministic fallback as Active Event detail, and shortens the resolved statistic label. Production remains unchanged pending public checkpoint and protected enhanced-only replacement.
...
35. `NEXT` — publish and independently verify the enhanced-dashboard correction; prove all active interface entities, including first observations with zero state changes, appear only under Interface Flaps; add latest AI summary with deterministic fallback to Active Event details; rename the resolved statistic to `Resolved`; replace only the enhanced resource after dry-run; and reverify all six resources, thirteen queries, original-dashboard exactness, and service health.

The end-to-end target remains complete. Item 35 is the single `NEXT`; it corrects only the enhanced NOC presentation without changing deterministic incident authority, transport, storage, model behavior, or the original dashboard.
