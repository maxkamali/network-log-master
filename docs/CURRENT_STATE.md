...
The environment transition, both component rebuild packages, guarded activation, component/cross-system runbooks, architecture reconciliation, final repository sanitation, repository/read-only-reference acceptance validation, final public milestone publication, clean-host risk disposition, collector-side production-normalizer integration, forward-only production cutover, multi-cadence stability review, deliberate transitional-parser retirement, deterministic incident engine, managed projection/incident boundary, deterministic wake-policy/compact-packet implementation, and guarded local-reasoning installation are complete. Item 29's backlog correction passed protected-copy rehearsal, exact inactive installation, protected resume, and one natural drain. The second corrected cadence kept packets fixed and pending fell 7→6, but strict validation rejected its model response. One terminal `INVALID_OUTPUT` run was stored without a result or invalid content, and reasoning was disabled. A protected-copy retry of the same packet reproduced the failure; a second copy-only diagnostic classified the defect as one action whose text exactly equaled a risk label. Production remains unchanged at 12 packets, six old-version pending, six runs, five successful results, one safe failure, zero `STARTED` rows/restarts, and zero deterministic lag. The first strengthened-schema gate correctly refused immutable-ID reuse; prompt revision `r2` then proved the local runtime does not enforce JSON Schema `not/enum` and safely failed the same packet on two protected copies. The 140-test `incident-assessment-v2-r3` candidate adds a portable 25-character generation minimum, longer than every risk label, while retaining both the negative enum and unchanged caller validator. It preserves all old provenance, exposes a clean 12-packet revised-version backlog, upgrades only the four exact predecessors, and is not installed. The LLM remains outside identity and lifecycle authority.
...
29. `NEXT` — publish and independently verify the portable-schema prompt revision, stage and retest its exact artifacts on GX10, then replay the same failed packet only on a protected current-state copy through `incident-assessment-v2-r3` and the unchanged caller validator. Only a successful copy replay may permit an exact inactive runtime-config/schema/caller/runner upgrade and a new protected production drain; all old runs/results and the historical terminal failure remain immutable evidence. Collector result return remains out of scope.
