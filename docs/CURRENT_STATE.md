...
29. `NEXT` — publish and independently verify the immutable-provenance compatibility correction, stage and retest its exact artifacts on GX10, then replay the same failed packet only on a protected current-state copy through prompt revision `incident-assessment-v2-r2`, the strengthened schema, and the unchanged caller validator. Only a successful copy replay may permit an exact inactive runtime-config/schema/caller/runner upgrade and a new protected production drain; all old runs/results and the historical terminal failure remain immutable evidence. Collector result return remains out of scope.
