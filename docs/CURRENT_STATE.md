...
30. `NEXT` — complete the separately gated GX10 result-return boundary. Local production and the active collector acceptance ledger are complete. The inactive sender package is installed with no private inputs. A separate configurator and configured-inactive verifier now pass 182 local and 182 exact GX10-staged tests: dedicated writer-role separation, atomic config-last installation, partial-state refusal, failure cleanup, fixed pinned-host/key contracts, and zero SFTP execution. Next publish this candidate, then generate/install only the dedicated writer identity and its matching collector authorization, verify the sender remains disabled/inactive, and publish the bounded first-live/replay plan. Live return must then prove collector validation, ClickHouse ingestion, complete provenance, exact replay suppression, and controlled failure isolation.
