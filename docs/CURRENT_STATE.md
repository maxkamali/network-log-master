...
The environment transition, both component rebuild packages, guarded activation, component/cross-system runbooks, architecture reconciliation, final repository sanitation, repository/read-only-reference acceptance validation, final public milestone publication, clean-host risk disposition, collector-side production-normalizer integration, forward-only production cutover, multi-cadence stability review, deliberate transitional-parser retirement, deterministic incident engine, managed projection/incident boundary, and deterministic wake-policy/compact-packet implementation are complete. Item 28 has published initial repository/guard candidates and a passing calibrated correction: the smallest model was rejected without weakening validation, while the exact Gemma candidate passed three isolated synthetic loopback cases. Protected-copy, unscheduled installation, and managed invocation gates remain. The LLM remains outside identity and lifecycle authority.
...
28. `NEXT` — publish and independently verify the calibrated Gemma/prompt/output-schema/caller/guard correction, then pass protected-production-state-copy migration, success, interruption, invalid-output, unavailable-runtime, and rerun gates. Do not install/schedule or invoke production-packet inference or add collector result return until those separate gates pass.
