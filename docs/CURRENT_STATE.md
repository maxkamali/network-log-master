...
28. `NEXT` — design and implement a versioned local model/prompt/structured-output execution boundary that consumes immutable reasoning packets without modifying deterministic incident or packet truth. Begin repository/copy-only, define durable run/idempotency and inference-unavailable behavior, and do not schedule production inference or add collector result return until separate gates pass.
