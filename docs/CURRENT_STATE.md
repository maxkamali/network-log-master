...
30. `NEXT` — complete the separately gated GX10 result-return boundary. Configured-inactive transport, first-live, exact-replay, and divergent-isolation checkpoints are published. Exact replay and conflict remain distinct; immutable ledger/ready and ClickHouse identities remain singular. A new explicit active-schedule verifier mode passes the 186-test local GX10 suite while the live sender timer remains disabled. Next publish/stage that verifier, pass the exact GX10 stage, then activate only the recurring sender timer with disable-on-failure and bounded natural-delivery/collector-ingestion proof.
