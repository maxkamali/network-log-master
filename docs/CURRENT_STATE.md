...
30. `NEXT` — complete the separately gated GX10 result-return boundary. Local production and the active collector acceptance ledger are complete. The inactive sender package is installed with no private inputs. The configured-inactive candidate passes 184 tests locally/on GX10 after optional-key-comment and captured-private-runtime portability corrections; the exact-backup collector authorizer passes five local/collector-runtime rollback tests. The dedicated collector authorization is installed under protected backup, while both GX10 configuration refusals occurred before target creation. Next publish the exact-hash captured-runtime derivation correction, configure GX10 while keeping the sender disabled/inactive, and independently verify both sides. Publish the bounded first-live/replay plan before transport. Live return must then prove collector validation, ClickHouse ingestion, complete provenance, exact replay suppression, and controlled failure isolation.
