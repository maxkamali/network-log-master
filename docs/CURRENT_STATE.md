...
30. `NEXT` — complete the separately gated GX10 result-return boundary. Local production and the active collector acceptance ledger are complete. The sender core/package passes 178 local and 178 exact GX10-staged tests. The exact package is now installed on GX10 in its independently verified inert state: timer disabled, service inactive, private config/key/known-hosts absent, active outbox at 20 ready/zero delivered, and all preexisting deterministic schedules healthy. Next implement and test a separate guarded private-input configurator/verifier while keeping the sender disabled. Do not transmit until configured-inactive verification passes and the bounded first-live/replay plan is published; later live return must prove collector validation, ClickHouse ingestion, provenance, replay suppression, and failure isolation.
