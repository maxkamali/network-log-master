...
26. `NEXT` — complete the inactive working-system install, explicit initial backfill, timer activation, and multi-cadence gates for the implemented managed canonical-projection/incident boundary. Private-copy full backfill/no-op/new-input/projection-failure/incident-failure evidence passes with exact ordering and zero cursor lag; Ollama invocation and result production remain excluded.
