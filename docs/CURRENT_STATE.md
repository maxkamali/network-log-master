...
30. `NEXT` — complete the separately gated GX10 result-return boundary. Local production and the active collector acceptance ledger are complete. The inactive sender package is installed with no private inputs. The configured-inactive candidate passes 183 tests locally/on GX10 after an optional-key-comment portability correction, and a separate exact-backup collector authorizer passes five local/collector-runtime rollback tests. The dedicated collector authorization is installed under protected backup; GX10 configuration safely refused before creating targets and awaits the published compatibility correction. Next publish that correction, configure GX10 while keeping the sender disabled/inactive, and independently verify both sides. Publish the bounded first-live/replay plan before transport. Live return must then prove collector validation, ClickHouse ingestion, complete provenance, exact replay suppression, and controlled failure isolation.
