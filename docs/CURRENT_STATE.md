...
29. `NEXT` — while reasoning remains disabled, implement and validate a bounded-backlog admission/coalescing correction that preserves all append-only packet/run/result state. Publish and rehearse it on a protected production-state copy, install it behind the disabled timer, then repeat protected activation and multiple natural cadences proving backlog cannot grow faster than the one-inference-per-cycle ceiling. Collector result return remains out of scope.
