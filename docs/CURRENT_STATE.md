...
30. `NEXT` — complete the separately gated GX10 result-return boundary. Local production is complete, and the durable collector acceptance ledger is active after all repository/staged/live bootstrap gates. The deterministic one-file sender core now passes 11 focused local tests and the same 11 tests from exact GX10-staged bytes, including strict pinned-host SFTP construction and interruption after transport before local acknowledgment; it is not installed and all transports were injected. Next package the sender behind an inactive exact-hash managed boundary without a populated private config or credential. Do not provision or transmit until the inactive package/protected-copy gates pass; later live return must prove collector validation, ClickHouse ingestion, provenance, replay suppression, and failure isolation.
