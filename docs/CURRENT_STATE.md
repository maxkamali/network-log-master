...
The environment transition, both component rebuild packages, guarded activation, component/cross-system runbooks, architecture reconciliation, final repository sanitation, repository/read-only-reference acceptance validation, final public milestone publication, clean-host risk disposition, collector-side production-normalizer integration, forward-only production cutover, multi-cadence stability review, deliberate transitional-parser retirement, deterministic incident engine, managed projection/incident boundary, deterministic wake-policy/compact-packet implementation, and guarded local-reasoning installation are complete. Item 29's first natural cadences exposed backlog growth, so reasoning was disabled. The published 138-test correction defers construction while backlog exists, passed protected-copy rehearsal, and was atomically installed while disabled with all reasoning rows unchanged. Protected resume then created a fresh backup, kept packets fixed at 12, reduced pending 9→8, advanced successful runs/results 3→4, and enabled the timer only after verification. Fetch/ingest, correlation, and Ollama remain healthy. The LLM remains outside identity and lifecycle authority.
...
29. `NEXT` — publish and independently verify the protected corrected-resume checkpoint, then collect multiple natural five-minute drain cadences. Each must keep packets fixed at 12 while pending falls by exactly one and runs/results rise by exactly one, with zero failure/`STARTED` rows/restarts, deterministic zero lag, and healthy independent timers. Collector result return remains out of scope.
