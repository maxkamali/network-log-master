...
29. `NEXT` — collect and independently verify multiple natural five-minute managed-reasoning cadences. Each must preserve one-inference maximum, zero unreconciled `STARTED` rows, zero restarts, deterministic zero lag, healthy independent production timers, and append-only result consistency before item 29 completes. Collector result return remains out of scope.
