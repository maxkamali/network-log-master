...
30. `NEXT` — complete the separately gated GX10 result-return boundary. Local production is complete after protected activation, natural no-op stability, and exact one-file catch-up. Next design and prove repository/copy-only write-only transport replay safety, explicitly closing the upload-success/local-acknowledgment crash window and collector duplicate behavior. Do not install writer credentials or transmit until those gates pass; later live return must prove collector validation, ClickHouse ingestion, provenance, and failure isolation.
