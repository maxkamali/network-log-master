...
28. `NEXT` — publish and independently verify the repository-only versioned local model/prompt/run/structured-output candidate, then add a guarded existing-system migration and pass synthetic local-model plus protected-production-state-copy gates. The candidate has durable crash-safe reservation/idempotency and explicit inference-unavailable behavior; do not schedule or invoke production-packet inference or add collector result return until separate gates pass.
