...
29. `NEXT` — publish and independently verify the strict generation-schema compatibility candidate, stage and retest its exact artifacts on GX10, then replay the same failed packet only on a protected current-state copy through the strengthened schema and unchanged caller validator. Only a successful copy replay may permit an exact inactive schema/caller/runner upgrade and a new protected production drain; the historical terminal production failure remains immutable evidence. Collector result return remains out of scope.
