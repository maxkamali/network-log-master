...
30. `NEXT` — complete the separately gated GX10 result-return boundary. Configured-inactive transport is complete and the first manual sender cycle moved exactly one of 59 ready files to delivered while keeping its timer disabled. Exact name/digest parity reached the collector, but the gate safely retained the file incoming: Linux protected-hardlink policy refused the gate account's cross-owner publication. The gate timer is stopped, ledger/ready/rejected remain zero, and an ownership-safe copy/no-overwrite-marker correction passes 11 tests locally and on the collector runtime. Next publish/install that correction under exact rollback, accept the retained file, prove ClickHouse provenance, then complete exact replay and divergent isolation before considering a recurring sender schedule.
