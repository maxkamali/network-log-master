...
The environment transition, both component rebuild packages, guarded activation, component/cross-system runbooks, architecture reconciliation, final repository sanitation, repository/read-only-reference acceptance validation, final public milestone publication, clean-host risk disposition, collector-side production-normalizer integration, forward-only production cutover, multi-cadence stability review, deliberate transitional-parser retirement, deterministic incident engine, managed projection/incident boundary, and deterministic wake-policy/compact-packet implementation are complete. Item 28 now has a published repository-only versioned local-model/prompt/run/structured-output candidate and a passing guarded-migration candidate; its real local-model evaluation, protected-copy, unscheduled installation, and managed invocation gates remain. The LLM remains outside identity and lifecycle authority.
...
28. `NEXT` — publish and independently verify the guarded existing-system migration candidate, then pass synthetic loopback local-model and protected-production-state-copy gates. The published caller candidate has durable crash-safe reservation/idempotency, explicit inference-unavailable behavior, and a migration guard that requires exact pre/post schemas, artifacts, protected backup, and zero scheduler references; do not schedule or invoke production-packet inference or add collector result return until separate gates pass.
