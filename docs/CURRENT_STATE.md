...
29. `NEXT` — publish and independently verify the second natural `r3` cadence, then observe at least one more independent timer cadence without manual service invocation under the same fixed-packet, exact-drain, one-success/result, single-historical-failure, zero-`STARTED`/restart/lag invariants. Disable only reasoning on any new failure. Collector result return remains out of scope.
