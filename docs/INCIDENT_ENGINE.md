...
`components/gx10/install/migrate-incident-engine.py` is the existing-system guard. It requires exact repository hashes, exact pre/post schema inventories, explicit root confirmation, an absent engine target, zero scheduler references, and protected destination parents. It creates and validates a root-only SQLite backup before applying the schema and installing the engine. Empty-state rollback removes only the new schema and engine; it refuses rollback after an incident or engine cursor exists.
