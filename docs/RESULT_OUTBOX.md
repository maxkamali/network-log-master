...
The local-producer milestone is complete. The collector-side durable acceptance ledger passes 11 focused local tests, the same 11 tests from exact collector-staged bytes, guarded production installation, independent empty-ledger verification, and one natural no-op cadence. It closes the upload-success/local-acknowledgment crash window by retaining immutable filename/content acceptance identity after ready-file removal and recovering publication-before-ledger interruptions. No writer credential or transmission exists. Continue with the deterministic sender repository/protected-copy gates described in `docs/RESULT_TRANSPORT.md`.
