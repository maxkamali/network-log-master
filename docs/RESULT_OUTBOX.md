...
The local-producer milestone is complete. The collector-side durable acceptance-ledger candidate now passes 11 focused local tests and the same 11 tests from exact collector-staged bytes. It closes the upload-success/local-acknowledgment crash window by retaining immutable filename/content acceptance identity after ready-file removal and recovering publication-before-ledger interruptions. The candidate is not yet installed; no writer credential or transmission exists. Continue with the guarded live collector-gate checkpoint, then build the deterministic sender repository/copy gates described in `docs/RESULT_TRANSPORT.md`.
