...
The local-producer milestone is complete. Next design repository/copy-only transport replay safety, including the upload-success/local-acknowledgment crash window and collector-side duplicate behavior. Do not install a writer credential or transmit until that design and its failure/replay tests pass.
