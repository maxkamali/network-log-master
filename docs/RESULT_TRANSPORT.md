...
Five new management tests cover config-last installation, failure cleanup, partial-state refusal, absence of an SFTP execution path, and optional public-key comment normalization. The resulting 183-test GX10 suite and filesystem/package contracts pass locally and from exact bytes in the retained temporary GX10 candidate tree. The first production configure attempt safely refused before creating any target because this runtime's `ssh-keygen -y` retains the private key's optional comment; the corrected parser binds only the key type/blob and ignores that nonidentity comment.
