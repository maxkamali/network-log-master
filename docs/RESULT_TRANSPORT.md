...
Six new management tests cover config-last installation, failure cleanup, partial-state refusal, absence of an SFTP execution path, optional public-key comment normalization, and exact captured-legacy-runtime derivation. The resulting 184-test GX10 suite and filesystem/package contracts pass locally and from exact bytes in the retained temporary GX10 candidate tree. The first production configure attempt safely refused because this runtime's `ssh-keygen -y` retains an optional comment. The second safely refused because the captured reference application predates the reconstructed JSON runtime config and retains endpoint/key paths as constants in the exact-hash fetcher. The corrected verifier supports either the clean-rebuild JSON or only the captured fetcher with its published SHA-256, strict root/mode/link metadata, AST-literal endpoint/key extraction, service-owned input metadata, and private-home containment.
