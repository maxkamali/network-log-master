...
3. package, rehearse, and install an independently managed inactive no-network local producer boundary
4. separately install the least-privilege write-only transport identity only after inactive producer verification passes
5. activate one bounded live result return and prove collector validation, ClickHouse ingestion, exact provenance, replay safety, and failure isolation
6. retain GX10 without direct ClickHouse access and keep all model results nonauthoritative
